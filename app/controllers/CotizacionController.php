...
class CotizacionController {
    ...
    private function procesarGuardarItem(int $cotizacion_id): void {
        if (!isset($_POST['csrf_token']) || !verificar_token_csrf($_POST['csrf_token'])) {
            header("Location: /PROYECTO_SODICOL/cotizaciones/crear_cotizacion.php?error=csrf");
            exit();
        }
        ...
        if (!in_array($iva, ['si', 'no'])) {
            header("Location: /PROYECTO_SODICOL/cotizaciones/crear_cotizacion.php?error=iva");
            exit();
        }
        ...
        header("Location: /PROYECTO_SODICOL/cotizaciones/crear_cotizacion.php");
        exit();
    }
    ...
    public function editarItem(): array {
        ...
        if (!isset($_SESSION['cotizacion_id'])) {
            header("Location: /PROYECTO_SODICOL/cotizaciones/crear_cotizacion.php?error=no_session");
            exit();
        }
            ...
            if (!isset($_POST['csrf_token']) || !verificar_token_csrf($_POST['csrf_token'])) {
                ...
                if (!in_array($iva, ['si', 'no'])) {
                    ...
                    if ($this->model->actualizarItem($itemId, $cotizacion_id, $titulo, $rutaFinal, $descripcion, $cantidad, $iva, $precio)) {
                        header("Location: /PROYECTO_SODICOL/cotizaciones/crear_cotizacion.php?updated=1");
                        exit();
                    }
        ...
        if (!isset($_GET['id']) || !validar_numero($_GET['id'])) {
            header("Location: /PROYECTO_SODICOL/cotizaciones/crear_cotizacion.php?error=invalid_id");
            exit();
        }
        ...
        if (!$datos) {
            header("Location: /PROYECTO_SODICOL/cotizaciones/crear_cotizacion.php?error=not_found");
            exit();
        }
    ...
    public function consultar(): array {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            if (!isset($_POST['csrf_token']) || !verificar_token_csrf($_POST['csrf_token'])) {
                ...
                header("Location: /PROYECTO_SODICOL/cotizaciones/consultar_cotizacion.php?buscando=1");
                exit();
            }
        }
